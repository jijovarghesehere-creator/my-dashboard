...
export async function fetchGlobalNews() {
  // Fetch top/global headlines from Google News via the proxy.
  // The parameters here (hl, gl, ceid) can be adjusted for localization.
  const rssPath = `/rss?hl=en-US&gl=US&ceid=US:en`
  const response = await fetch(`/api/news${rssPath}`)
  if (!response.ok) throw new Error('Could not load global news.')
